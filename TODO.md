...
- [ ] [2026-09-15] La tâche en cours (/live, cadre vert) ne survit pas à un redémarrage du webhook : CURRENT_TASK est en mémoire. Fix : fichier d'état DATA/current_task.json écrit/supprimé par _update_current_task, relu au chargement ; garde-fou si le finish a été manqué (start + seconds dépassé). Réutilisable par le timer (C9 de specs/timer-discussion.md).
