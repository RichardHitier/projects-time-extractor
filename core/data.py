...
def read_pomo(pomo_file: str) -> pd.DataFrame:
    ...
    Returns:
        DataFrame with columns: date, startTime, endTime, project, task, minutes.
    """
    if not os.path.exists(POMO_FILE):
        print(
            f"Pomofocus export not found: {
                POMO_FILE}." " Run 'timer pomo-merge' first."
        )
        return
...
def load_all_pomo():
    ...
    df["minutes"] = pd.to_numeric(
        df["minutes"], errors="coerce").fillna(0).astype(int)
